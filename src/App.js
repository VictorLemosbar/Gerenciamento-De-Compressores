...
function MyApp() {
  ...
  //UseState que verifica o estado do botao desligar
  const [powerOffControl, setPowerOffControl] = useState(false)

  //UseState que verifica o estado do botao ligar
  const [powerOnControl, setPowerOnControl] = useState(false)

  useEffect(() => {

   if (powerOffControl === true) {
    alert("Não Foi possível desligar o compressor")
    setPowerOffControl(false)
   }

   if (powerOnControl === true)  {
    alert("Não Foi possível ligar o compressor")
    setPowerOnControl(false)
   }

  }, [powerOffControl, powerOnControl])
  ...
  //Function temporaria para atribuir valores aos botoes de estado ligar e desligar
  return (
    <div id="app">
      ...
      <div id="menuDireito">
        <div id="botoesEstado">
          <button onClick={() => setPowerOnControl(true)} id="ligar">
          </button>
          <button onClick={() => setPowerOffControl(true)} id="desligar"></button>
        </div>
      </div>
